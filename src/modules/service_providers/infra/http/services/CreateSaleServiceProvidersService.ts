...
interface ICreateProvidersResponse {
  created_providers: ServiceProvider[];
}
...
class CreateSaleServiceProvidersService {
  ...
  public async execute({
    ...
    const created_providers: ServiceProvider[] = [];
    ...
    for (const sale_id of sale_ids) {
      ...
      if (saleById) {
        if (techinical_comments) {
          saleById.techinical_comments = techinical_comments;
          saleById.production_status = 'TO_DO';

          await this.saleRepository.save(saleById);
        }
