...
interface IGeneratePdfReportServiceParams {
  initialDate?: Date;
  finalDate?: Date;
  company?: string;
  status?: string;
...
interface IGeneratePdfReportServiceResponse {
...
class GeneratePdfReportService {
  ...
  public async execute({
    ...
    finalDate,
    status,
  }: IGeneratePdfReportServiceParams): Promise<IGeneratePdfReportServiceResponse> {
    ...
    if (company) {
      companyById = await this.companyRepository.findById(String(company));
    }
...
export default GeneratePdfReportService;
